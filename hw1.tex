...
If using $L_2$, the contrain is a circle as the form of the regularization term.
...
The posterior can be written as $p(f\mid \mathbf{t})$ which means given certain data the
probability of a random variable $f$. If there is no given data, then obviously
the probability of a random variable $f$ is $p(f)$ which is the prior.

% The predictive posterior distribution of the model:
\end{question}
