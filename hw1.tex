...
If we consider the noise of each observation as a random variable, then the sum
...
Choosing $L_1$ norm as regularizer is known as the lasso (least absolute shrinkage and selection operator).
The regularizer will force some weighting coefficients $w_j$ to be zero, given a proper choice of the model parameter $\tau$. It plays the role of feature selection
since those zero weighting coefficients indicate that the corresponding features
are irrelevant to the output.

The penalization term in the negative log-prior:
\begin{align*}
...
we can see that in the corresponding minimization problem (i.e. least squares problem).
The regularization term acts as a constraint in the minimization problem.

In the right head side of the figure, it is a minimization problem using $L_1$
regularization term.
If using $L_1$ norm, the minimization result $\mathbf{w}^{*}$ is forced to lay on the 
$w_2$ axis and the parameter $w_1$ of $\mathbf{w}^{*}$ becomes zero and which
can be understood as feature selection. In the figure, the constraint is a square
shape contour.

If using $L_2$ norm, the constraint is a circle as the form of the regularization 
term. All components of $\mathbf{w}^{*}$ will be forced to be zeros.
...
\begin{align*}
  \textbf{T} &= \textbf{WX} + \textbf{E} \\
  \textbf{E} &= [\bm{\epsilon}, \bm{\epsilon}, .., \bm{\epsilon}]
\end{align*}
...
Consider the equation 7 in the question with respect to the parameters $\textbf{W}$
 and compare the quadratic and linear terms of the exponents both sides:
\begin{align*}
  p(\textbf{W}\mid \textbf{X}, \textbf{T}) =&
...
The constant $Z$ plays no role in the calculation since only the quadratic and linear terms are considered.
% Substitute back \eqref{eq:q5-covar} and \eqref{eq:q5-mean} into the posterior
...
the result is trivial that we can obtain the predictive mean function by adding the mean-removal result to the fixed mean function.
...
well. The specification of the covariance function indicates a distribution over functions. To visualize it, we can draw some samples from the distribution of function evaluated at some test input points.
...
The marginalization connects the prior and the likelihood of the data. The
integral is a weighted average averaging the likelihood of the data with the prior
as weighting.

The uncertainties of the data and the prior are combined or added up into the uncertainty of the marginalization. The covariance of the marginalisation is
...
The hyperparameters $\bm{\theta}$ still condition on the part of covariance of the marginalization from the prior. That part of covariance is the kernel function.
\end{question}
...
When more data observed, the posterior becomes concentrated over the parameter space.
In general, we can say that the posterior receive more information about the data.
Therefore, the posterior can have more confidence to tell what is the underlying parameter behind the data. 
On the other hand, the posterior Gaussian covariance could be expressed as 
...
larger and the determinant of the covariance matrix will be smaller.
...
Consider the noise with different values of $\sigma$, the spreading of the probability distribution of posterior over $\mathbf{W}$ increases as the sigma
value increases. As the uncertainty of posterior increases, the models or the resulting functions variate more.
\end{question}
...
We draw 10 samples from this prior for 4 different length scales in figure \ref{fig:Q10-prior}.

For the case $l = 0.05$, the correlations among points tend to zero. Therefore,
the prior behaves as white noise.

When the length scale increases, the correlation between each pair of points increases.
...
Sample functions, predictive mean and variance of the posterior are shown in figure \ref{fig:Q11-predict-post}. 
The black dash lines are the predictive mean functions. 
The blue shadows are the range from $\mu - \sigma$ to $\mu + \sigma$.
...
$K(\mathbf{X}, \mathbf{X})$ to be a zero matrix. 
The sample functions alter a bit compared to the prior since the strong assumption
on the correlation of points. In other words, the prior indicates that it is a 
restrictive model and therefore data cannot alter the model too much.
...
\end{align*}
It is equivalent to considering the sample noise. The predictive posterior will
...
\begin{question}{12}
This prior encodes that the mean of $\mathbf{X}$ is a zero matrix and each element
in $\mathbf{X}$ are independent.
It is because we have no information about the latent variable $\mathbf{X}$.
Therefore, we assume that it is a zero-mean unit-covariance Gaussian. We do that because
...
Because $p(\mathbf{X})$ and $p(\mathbf{Y}\mid\mathbf{X}, \mathbf{W})$ are Gaussian, 
 ...
 Furthermore, we can consider each $\mathbf{y}_i$ are independent. Therefore, 
 we can write:
\begin{align*}
...
\begin{align*}
  ...
  &= \operatorname{argmin}_{\mathbf{W}}
    [\sum_{i=1}^{N} \mathbf{y}_i^T \mathbf{C}^{-1}\mathbf{y}_i
      + N\log{|\mathbf{C}|}]
\end{align*}
Type-II maximum likelihood approach does not depend on $\mathbf{X}$ and it is
...
The last two expressions of Eq. 25 are equivalent. It is because the denominator:
  \begin{align*}
...
The Type-II Maximum-likelihood is a sensible approach since it considers how to choose a parameter $\mathbf{W}$ which maximizes the model evidence. 
In other words,
it is in the sense that we choose a parameter to maximize the probability of getting such a set of observations.
\end{question}
...
Below is the derive of the gradient in the element-wise approach. The Matrix cookbook 2012 Ed.
...
\end{align*}
Consider the derivative of $\frac{N}{2}\log(|\mathbf{C}|)$:
\begin{align*}
...
\end{align*}
Consider the derivative of $\frac{N}{2}\text{Tr}(\mathbf{C}^{-1}\mathbf{S})$:
...
\begin{question}{16}
Figure \ref{fig:q16-replearn} shows both the learned and the original 
...
and the original representation. It is because we have a degree of freedom to choose
...
The scale of the learned representation is magnified for easier comparison.
\begin{figure}[h!]
  \centering
...
\begin{question}{17}
It is the simplest model since it has zero degrees of freedom to change the model.
The model implies that there is an equal probability of generating different kinds of
data. To answer whether a model is a good or bad model, it depends on the evidence
of the data. 
...
\begin{figure}[h!] %TODO: fix the figure. model1 -> model 100
  ...
  \caption{Explanation of a good model and a bad model}
...
For example, If $x^n_1$ represents the column index of the grid, then only whole column has the same output can be explained by this model.
Therefore, the model $M_1$ has a small spreading of its probability mass over 
$\mathcal{D}$.
\end{question}

\begin{question}{19}
The terms in the exponent of the logistic equation can be considered as a linear separation plane.
$M_2$ and $M_3$ are considered as more flexible linear separation planes on the grid.
...
To the number of data sets each model can explain, $M_3$ can explain the most, 
then $M_2$ and then $M_1$.
The uncertainty of a model can be considered as ($1 - p(d_i\mid M_i)$),
where $d_i$ is a data sets.
...
it has no model parameter.

In terms of the number of data set a model can explain, $M_0$ is the most
flexible since it can explain all possible data by the uniform probability distribution.
$M_3$ is more flexible than $M_2$ and $M_1$. $M_2$ can explain data which can also be explained by $M_3$ except one situation. That situation is all
...
That variable usually is what we are not interested in our model.

The implication of the process is to calculate the weighted average of the model
likelihood according to the prior on the parameters.

The evidence can also be regarded as the probability of generating the data set 
$\mathcal{D}$ from a model described by all possible parameters. 
That is the reason why we do weighted average on the likelihood.
\end{question}

\begin{question}{21}
First, the spherical Gaussian prior implies that all components of the weighting
vector $\bm{\theta}$ are independent.

Second, a large variance of the weighting factor allows sampling a weighting vector
with large L2-norm from the distribution. 
In the sense of SVM, the margin of the separation plain is inversely proportional to the L2-norm of the weighting vector. Therefore, we
allow the separation plane with a large uncertainty and high flexiblity.
\end{question}

\begin{question}{22}
The sum of the evidence over all possible data sets which belong to $\mathcal{D}$ is 1. 
It means given a model $M_i$ sum the probability over all possible data sets and it obviously equals to 1. It also means that each
...
$M_0$ spreads its probability mass over the whole region of data set. And the dataset
region covered by $M_3$ is greater than $M_2$ and the dataset region of $M_2$ 
is greater than $M_1$.

We can consider the parameterization scheme employed by each model. 
$M_1$ represents
a vertical separation plane (if we consider $x_0$ as the column index of the grid).
The model is simple enough so that the amount of data set can be explained by this
model is limited. 
$M_2$ can be viewed as a separation plane passing through the origin and $M_2$ is then 
flexible than $M_1$. Thus the region covered by $M_2$ is larger by $M_1$.
$M_3$ is more flexible since it is a separation plane allowed to have offset from the
origin. Hence, $M_3$ can explain all linear separable data.
...
In the table (\ref{subtable:M1-low}), we can see that it is not possible to draw any
vertical line to separate crosses and circles.
...
center of the grid.
It is possible to use a diagonal line from left-bottom to up-right to sperate crosses
and circles as shown in the table (\ref{subtable:M2-high}). 
While it is not possible to use any line passing through the center to sperate crosses and circles in the table (\ref{subtable:M2-low}).
...
cases with its bias term. Moreover, the prior has the preference for choosing large
parameters with its large variance. Therefore, the prior makes the separate line 
to have a large intercept and the line most likely separate the data shown in table 
(\ref{subtable:M3-high}).
The peak of $M_3$ in 
figure \ref{subfig-2:plotEvidSub} can also be explained by all circles and all crosses. 
Therefore, the model evidence of all circles or crosses is the highest at $M_3$.
...
If we use an extremely small $\sigma^2$, says $\sigma^2 = 10^{-6}$, we would probably
...
arbitrary large number from the prior. However, the separation line slope depends
...
constraints over parameters. The marginalisation carries the this information to 
the model evidence $p(\mathcal{D}\mid M_i)$. Therefore, we can consider
that the models $\{{M_i}\}_{i=1}^3$ becomes restrictive under the dependent choice of parameter compared to independent choice. 
Each model covers a smaller amount of data set as shown in figure \ref{fig:Q24-non-diag-plotEvid}.
\begin{figure}[h] 
  \centering
...
\ref{fig:Q24-non-zero-plotEvid}. Those data sets are diagonal separable since we make a preference on the choice of parameter.

The region covered by each model should be almost the same as the zero-mean prior.
It is because the flexibility of models holds when changing the mean of prior.
