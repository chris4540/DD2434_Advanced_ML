...
\end{align*}
where $\tau_{nk} = \mathbb{E}[c_nk] = p(C^n = k \mid X^n, \theta^{(t)})$ is the
...
\begin{align*}
    Q_{\pi^{t}}(\pi) &= \sum_{n=1}^{N} 
        \tau_{nk} \mathbb{E}_{p(Z, C\mid X, \bm{\theta}^{(t)})}[\log \pi_k]  \\
...
\begin{equation}
    \pi_k = \frac{\sum_{n=1}^{N} \tau_{nk}}{N} \label{eq:Q27-update_pi_k}
\end{equation}

\begin{align*}
    Q_{\lambda^{t}}(\lambda) &= \sum_{n=1}^{N} 
        \tau_{nk} \mathbb{E}_{p(Z, C\mid X, \bm{\theta}^{(t)})}[\log \lambda^k_{Z^n_1}] \\
\end{align*}
\end{problem}
\pagebreak
