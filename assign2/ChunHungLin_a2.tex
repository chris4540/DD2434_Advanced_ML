...
\begin{align*}
    ...
    &= \prod_{n=1}^{N} [\pi_k p(X^n_{1:M}, Z^n_{1:M}\mid \lambda^k, A^{k}, e^{k})]^{c_{nk}} \\
\end{align*}
...
\begin{align*}
    ...
    &= \sum_{n=1}^{N} \log [(\pi_k p(X^n_{1:M}, Z^n_{1:M}\mid \lambda^k, A^{k}, e^{k}))^{c_{nk}}] \\
    &= \sum_{n=1}^{N} [c_{nk}\log \pi_k + c_{nk}\log p(X^n_{1:M}, Z^n_{1:M}\mid \lambda^k, A^{k}, e^{k})]  \\
    &= \sum_{n=1}^{N} 
        [c_{nk}\log \pi_k 
...
\begin{align*}
    Q_{\pi^{t}}(\pi) &= \sum_{n=1}^{N} 
        \tau_{nk} \mathbb{E}_{p(Z\mid X, \bm{\theta}^{(t)}_k)}[\log \pi_k]  \\
    &=  \sum_{n=1}^{N} \tau_{nk} \log \pi_k && \text{(Since $\pi_k$ is not a random variable)}\\
\end{align*}
...
\begin{align*}
    \mathcal{L} = \sum_{n=1}^{N} \tau_{nk} \log \pi_k - g(\sum_{k=1}^{K} \pi_k - 1)
\end{align*}
...
\begin{align*}
    Q_{\lambda^{(t)}}(\lambda) &= \sum_{n=1}^{N} 
        \tau_{nk} \mathbb{E}_{p(Z\mid X, \bm{\theta}^{(t)}_k)}[\log \lambda^k_{Z^n_1}] \\
    &= \sum_{n=1}^{N} \tau_{nk} \mathbb{E}_{p(Z\mid X, \bm{\theta}^{(t)}_k)}
        [\sum_{r=1}^{R}\mathbb{I}(Z^n_1 = r)\log \lambda^k_{r}] \\
    &= \sum_{n=1}^{N} \tau_{nk} \sum_{r=1}^{R} 
...
For updating $A$:
\begin{align*}
    Q_{A^{(t)}}(A) &= \sum_{n=1}^{N} \tau_{nk} \sum_{m=2}^{M} 
        \mathbb{E}_{p(Z\mid X, \bm{\theta}^{(t)}_k)}[\log A^k_{Z^n_{m-1},Z^n_m}] \\
    &= \sum_{n=1}^{N} \tau_{nk} \sum_{m=2}^{M} 
    ...
    &= \sum_{n=1}^{N} \tau_{nk} \sum_{m=2}^{M} \sum_{i,j=1}^{R} \xi_{m}^{n;k}(i,j)\log A^k_{i,j}
\end{align*}
where
\begin{align*}
...
\begin{align*}
    Q_{e^{(t)}}(e) &= \sum_{n=1}^{N} \tau_{nk} \sum_{m=1}^{M} 
