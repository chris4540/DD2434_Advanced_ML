...
\begin{align*}
    ...
    q(X, Z) &= q(Z_{1})\prod_{m}q(Z_{m+1}\mid Z_{m})\prod_{m}q(X_{m}\mid Z_{m})
\end{align*}
...
\begin{align*}
    ...
    &= \sum_{r=1}^{R}\sum_{j=1}^{R}\sum_{m=2}^{M}q(Z^n_{m+1}=j \mid Z^n_{m}=r)\log A^k_{r,j}
\end{align*}
...
\begin{align*}
    &\sum_{n,k}\mathbb{E}_{q(C)}[c_{nk}]\mathbb{E}_{q(Z)}[\sum_{m=2}^{M}\log A^k_{Z^n_{m}Z^n_{m+1}}] \\
    &= \sum_{n,k} r_{nk} \sum_{r=1}^{R}\sum_{j=1}^{R}\sum_{m=2}^{M}q(Z^n_{m+1}=j \mid Z^n_{m}=r)\log A^k_{r,j} \\
    &= \sum_{k}\sum_{r}\sum_{j}(\sum_{n}\sum_{m=2}r_{nk} q(Z^n_{m+1}=j \mid Z^n_{m}=r))\log A^k_{r,j}
\end{align*}
...
with the parameters:
\begin{equation}
    \tilde{\alpha_{A}}^k_{r,j} = \alpha_{A,j} + \sum_{n=1}^{N}\sum_{m=2}^{M}r_{nk} q(Z^n_{m+1}=j \mid Z^n_{m}=r)
\end{equation}
...
\begin{align*}
    \mathbb{E}_{q(Z, X)}[\mathbb{I}(Z^n_{m}=i, X^n_{m}=r+l)] = q(X^n_{m}=r+l \mid Z^n_{m}=i)
\end{align*}
Then, we have:
\begin{align*}
    ...
    &= \sum_{i=1}^{R}\sum_{l=-\Delta}^{\Delta}q(X^n_{m}=r+l \mid Z^n_{m}=i)\log e^k_{i,r+l} \\
\end{align*}
Therefore,
\begin{align*}
    &\sum_{n,k}\mathbb{E}_{q(C)}[c_{nk}]\sum_{m=1}^{M}\mathbb{E}_{q(Z)q(X)}[\log e^k_{Z^n_{m}X^n_{m}}]\\
    &= \sum_{n,k}r_{nk}\sum_{m=1}^{M}\sum_{i=1}^{R}\sum_{l=-\Delta}^{\Delta}q(X^n_{m}=r+l \mid Z^n_{m}=i)\log e^k_{i,r+l} \\
    &= \sum_{k=1}^{K}\sum_{i=1}^{R}\sum_{l=-\Delta}^{\Delta}
        (\sum_{n=1}^{N}\sum_{m=1}^{M}r_{nk}q(X^n_{m}=r+l \mid Z^n_{m}=i))\log e^k_{i,r+l}
\end{align*}
...
with the parameters:
\begin{equation}
    \tilde{\alpha_{e}}^k_{r,l} = \alpha_{e,l} + \sum_{n=1}^{N}\sum_{m=1}^{M}r_{nk} q(X^n_{m}=r+l \mid Z^n_{m}=i)
\end{equation}
...
\begin{align*}
    \log q(\pi) &= \mathbb{E}_{q(Z,X)q(C)q(A)q(e)}[\log p(X, Z, C, \Theta, \pi)] + \text{const} \\
...
% Update Q(X, Z)
\begin{flushleft}
    \textbf{\underline{Update $q(X, Z)$}}
\end{flushleft}
\begin{align*}
    \log q(X, Z) &= \mathbb{E}_{q(\pi)q(C)q(A)q(e)}[\log p(X, Z, C, \Theta, \pi)] + \text{const} \\
...
\begin{align*}
    q(X, Z) = \prod_{n,k}(\lambda^k_{Z^n_{1}}
        \prod_{m=2}^{M}\tilde{A}^k_{Z^n_{m}Z^n_{m+1}}\prod_{m=1}^{M}\tilde{e}^k_{Z^n_m X^n_m})^{r_nk}
\end{align*}
